...
//						this.contentList.append
        ...
        getPostData(searchContext){
            ...
            axios.post('http://132.232.247.209:9200/subwaycode02/_search', {
                "query":{
                    "match":{
...
//						    //获取当前查询到的所有条目总数
            var numsOfSearch_get = response.data.hits.total.value;						   	
            this.numsOfSearch = numsOfSearch_get;
            console.log("当前获取到的总消息条目数为:" + numsOfSearch_get);
            //获取每一个查询到的数组 并存储显示
            for(let i=0;i<numsOfSearch_get;i++){
                //console.log(response.data.hits.hits[i]._source)
                try{
                    var tempVar = (response.data.hits.hits[i]._source)
                    //console.log(tempVar)
                    //获取到的每条消息的内容
